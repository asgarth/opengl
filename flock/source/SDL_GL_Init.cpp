...
bool SDL_GL_Init::sdl_init (const char *title, int w, int h, int b, bool fs) {
	...
	video_flags |= SDL_OPENGL | SDL_HWPALETTE | SDL_GL_DOUBLEBUFFER;
	...
	SDL_ShowCursor(1);								// show mouse cursor
...
void SDL_GL_Init::opengl_init (void) {
	...
	glDisable (GL_DEPTH_TEST);															// profondità (disabilitato per blending)
...
void SDL_GL_Init::release_memory () {
	if (screen)
