...
bool OpenGL_Genesis::Enable_OpenGL (WNDPROC WndProcTT, int width, int height, int bits)
{
	...
	if (MessageBox(NULL, " Eseguire in FullScreen ? ", " Genesis - Fullscreen ", MB_YESNO | MB_ICONQUESTION) == IDNO)
		fullscreen = false;														// modalità finestra

	if (!CreateGLWindow (" - Genesis (OpenGL) - ", width, height, 32, fullscreen))				// crea la finestra OpenGL
	{
